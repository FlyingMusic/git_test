...
char* MD5::getMD5(char *str)
{
    ...
    if (m_md5Str == NULL) {
        m_md5Str = (char *)malloc(sizeof(char) * 33);
        if (m_md5Str == NULL) {
            printf("m_md5Str malloc failed");
            return NULL;
        }
    }
    //LOGI("加密前:%s",str);
    for(i=0;i<16;i++)
    {
       // LOGI("%02x",decrypt[i]);  //02x前需要加上 %
        sprintf(m_md5Str+i*2, "%02x", decrypt[i]);
    }
    m_md5Str[32] = 0;
    //LOGI("加密后32位:%s", m_md5Str);
    return m_md5Str;
}
